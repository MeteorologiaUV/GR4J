...
def Cemaneige(df, theta1: float, theta2:float, alfa_PP1:float, alfa_PP2:float, alfa_PP3:float, Tbias:float ):
    lat = df.loc[df.index[0],'gauge_lat']*2*np.pi/360 
    df['PET_bias'] = 0.408*0.0023*(df['Tmax-0'] - df['Tmin-0'])**0.5*(0.5*df['Tmax-0'] + 0.5*df['Tmin-0'] + 17.8 + Tbias)
    df['PET_bias']  = 3.7595*10*(df.hs*np.sin(lat)*np.sin(df.gamma)+np.cos(lat)*np.cos(df.gamma)*np.sin(df.hs))*df.PET_bias
...
def GR4J_local(df, alfa_PP1:float, alfa_PP2:float, alfa_PP3:float, theta1: float, theta2:float, x1: float, x2:float, x3:float, x4:float, Tbias:float):

    
    [rows, cols] = df.shape

    df_i = Cemaneige(df, theta1, theta2, alfa_PP1, alfa_PP2, alfa_PP3, Tbias)       
...
def GR4J_eval(ID, period:str):
    ...
    parameters = np.zeros((num_calibrations,14)) # OF + n° parameters
    parameters = pd.DataFrame(parameters, columns=['alfa_PP1','alfa_PP2','alfa_PP3','theta1', 'theta2', 'x1', 'x2', 'x3', 'x4','Tbias','code','KGE_train','KGE_eval','iter']) 
    
    
    for j in tqdm(range(num_calibrations)):
    
        full_path = path_par + '/' + str(ID) + '_iter_b_' + str(j) + '.par'
        df_p = pd.read_csv(full_path)
        df_p.drop(columns=['code.1'], inplace=True)
        
        parameters.at[j,'code'] = df_p.loc[0,'code']
        parameters.at[j,'alfa_PP1'] = df_p.loc[0,'alfa_PP1']
        parameters.at[j,'alfa_PP2'] = df_p.loc[0,'alfa_PP2']
        parameters.at[j,'alfa_PP3'] = df_p.loc[0,'alfa_PP3']
        parameters.at[j,'theta1'] = df_p.loc[0,'theta1']
        parameters.at[j,'theta2'] = df_p.loc[0,'theta2']
        parameters.at[j,'x1'] = df_p.loc[0,'x1']
        parameters.at[j,'x2'] = df_p.loc[0,'x2']
        parameters.at[j,'x3'] = df_p.loc[0,'x3']
        parameters.at[j,'x4'] = df_p.loc[0,'x4']
        parameters.at[j,'Tbias'] = df_p.loc[0,'Tbias']
        
        parameters.at[j,'KGE_train'] = df_p.loc[0,'KGE_train']
        ...
        df = load_data(ID)
        sim, obs, df = GR4J_local(df, df_p.alfa_PP1[0], df_p.alfa_PP2[0], df_p.alfa_PP3[0], df_p.theta1[0], df_p.theta2[0], df_p.x1[0], df_p.x2[0], df_p.x3[0], df_p.x4[0],df_p.Tbias[0])
        sim, obs, df = spliting(df, sim, obs, period) 
        parameters.at[j,'KGE_eval'] = KGE_metric(sim,obs)


    
    parameters.sort_values(by=['KGE_eval'], ascending=False, inplace=True)
    filepath = path_par + '/' + str(ID) + '_b_eval.csv'
