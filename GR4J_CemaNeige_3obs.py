...
class spotpy_setup():
    def __init__(self,c,dim=9):
        self.dim = dim
        self.catchment = c
        self.params = [spotpy.parameter.Uniform('a1',low=0, high=2.0, optguess=0.33),
                       spotpy.parameter.Uniform('a2',low=0, high=2.0, optguess=0.33),
                       spotpy.parameter.Uniform('a3',low=0, high=2.0, optguess=0.33),
                       spotpy.parameter.Uniform('Tg1',low=0.1, high=7, optguess=3), #degree-day factor
                       spotpy.parameter.Uniform('Tg2',low=0, high=1, optguess=0.5), #x6: snowpack inertia factor
                       spotpy.parameter.Uniform('x1',low=1, high=5000, optguess=1500.0), #x1: Capacity of production store (mm)
                       spotpy.parameter.Uniform('x2',low=-10, high=10, optguess=-2.0), #x2: Water exchange coefficient (mm)
                       spotpy.parameter.Uniform('x3',low=1, high=1500, optguess=200.0), #x3: Capacity of routing store (mm)
                       spotpy.parameter.Uniform('x4',low=0.501, high=4.5, optguess=1.6), #x4: UH time base (days)
                       ]
    ...
    def simulation(self, vector):
        ...
        sim, obs = self.model.GR4J_local(alfa_PP1 =vector[0], alfa_PP2 =vector[1], alfa_PP3 =vector[2], theta1 = vector[3], theta2 = vector[4], x1=vector[5], x2=vector[6], x3=vector[7], x4=vector[8])
...
class model():
    ...
    def Cemaneige(self, theta1: float, theta2:float, alfa_PP1:float, alfa_PP2:float, alfa_PP3:float ):
        df_i = self.df
        alfa_IMERG = alfa_PP1
    ...
    def GR4J_local(self, alfa_PP1:float, alfa_PP2:float, alfa_PP3:float, theta1: float, theta2:float, x1: float, x2:float, x3:float, x4:float):

        
        [rows, cols] = self.df.shape

        df_i = self.Cemaneige(theta1, theta2, alfa_PP1, alfa_PP2, alfa_PP3)       
...
def GR4J_snow_bias(c:int):
    ...
    camel_path = current_path #'/groups/hoshin/ldelafue/GR4J/'
    global it    
    global warm_up
    warm_up = 365
    global spotpy_setup
    ...
    for it in range(iterations):
        ...
        parameters = np.zeros((1,11)) # OF + n° parameters
        parameters = pd.DataFrame(parameters, columns=['alfa_PP1','alfa_PP2','alfa_PP3','theta1', 'theta2', 'x1', 'x2', 'x3', 'x4', 'code','KGE']) 
        ...
        parameters.at[0,'KGE'] = 0 
        parameters.index = parameters.code
        print(parameters)
        
        filepath = camel_path + '/results/3PP_model_KGE/' + str(c) + '_iter' + str(it) + '.par' #CHECK FOLDER
